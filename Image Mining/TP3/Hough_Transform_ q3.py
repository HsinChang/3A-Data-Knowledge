...
while (1):
    ...
    if ret == True:
        ...
        orientation = (np.arctan2(dx, dy)+np.pi)/(2*np.pi)
        magnitude = cv2.magnitude(dx, dy)
        mask = np.where(magnitude< THRESHOLD, 0 ,1)
        masked = np.multiply(orientation, mask)*255
        red = cv2.cvtColor(masked.astype('uint8'), cv2.COLOR_GRAY2RGB)
        red[mask==0] = [0,0,255]

        cv2.imshow("dsg",red)
        cv2.imshow("dg", orientation)
